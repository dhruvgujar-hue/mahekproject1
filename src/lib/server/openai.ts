...
export async function editImageWithBlob(
  blob: Blob,
  prompt: string,
  options?: { size?: '1024x1024' | '1024x1536' | '1536x1024'; timeoutMs?: number }
): Promise<string> {
  ...
  try {
    const data = await typedFetch<{ data?: { b64_json?: string; url?: string }[] }>(`${OPENAI_BASE_URL}/v1/images/edits`, {
      ...
      // Help longer-running upstream requests survive proxies/agents
      // (supported by undici/node-fetch in Node runtimes)
