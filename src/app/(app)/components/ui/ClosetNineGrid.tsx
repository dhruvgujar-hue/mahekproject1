...
import { getBaseImageKey, getResultsByPair } from '@/lib/services/tryOnRepo';
...
export default function ClosetNineGrid() {
    ...
    React.useEffect(() => {
      const unsub = tryOnQueue.onChange((job) => {
        if (job.status === 'succeeded' && job.images && job.images.length > 0) {
          if (job.itemId === pendingItemId) setPendingItemId(null);
          const currentPhase = useGameStore.getState().phase;
          if (currentPhase === 'StylingRound') {
            const currentBase = useGameStore.getState().currentImageUrl;
            if (!currentBase || currentBase !== job.baseImageUrl) return;
        ...
        if (job.status === 'failed' && job.itemId === pendingItemId) {
  ...
  async function applyAllOnModel(): Promise<void> {
    if (applyingAll) return;
    const state = useGameStore.getState();
    const startBase = state.currentImageUrl || baseUrl;
    if (!startBase || wardrobe.length === 0) return;
    const clothingImageUrls = Array.from(new Set(wardrobe.map((w) => w.imageUrl)));
    setApplyingAll(true);
    try {
      let currentBase = startBase;
      for (const clothingImageUrl of clothingImageUrls) {
        const images = await performTryOn(currentBase, clothingImageUrl);
        const nextImage = images[0];
        if (!nextImage) continue;
        await selectImage(nextImage, { type: 'tryOn', description: 'Batch try-on result', addToHistory: true });
        currentBase = nextImage;
      }
    } catch (err) {
            ...
            <div key={idx} className={phase === 'StylingRound' 
              ...
              <div
                ...
                onClick={() => { if (item && phase === 'StylingRound') { void (async () => {
                  ...
                  const state = useGameStore.getState();
                  const baseImageId = state.currentImageId;
                  const clickedBaseUrl = state.currentImageUrl || baseUrl;
                  if (!clickedBaseUrl) {
                    setPendingItemId(null);
                    return;
                  }
                  const baseImageKey = getBaseImageKey(baseImageId ?? null, clickedBaseUrl);
                  const cachedForCurrentBase = await getResultsByPair(baseImageKey, item.id);
                  if (cachedForCurrentBase?.[0]) {
                    await selectImage(cachedForCurrentBase[0], { type: 'tryOn', description: 'Applied from wardrobe', addToHistory: true });
                    setPendingItemId(null);
                    return;
                  }
